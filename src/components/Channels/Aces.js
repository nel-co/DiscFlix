...
export default class Aces extends React.Component {

  handleVideoClick = (e) => {
    e.stopPropagation();
    e.currentTarget.parentNode.parentNode.style.borderBottom = '3px solid #ffef00';
    this.props.handleDefaultHistory(e,this.props.aceVideos);
  }
  ...
  render() {
    let channelItems = this.props.aceVideos.map((item, index) => {
        ...
        <div 
          ...
          onMouseLeave={this.handleVideoLeave}>
            ...
            <div style={globalStyles.VideoOverlayStyle} className="VideoOverlay">
              ...
              <span className="icon-btn" onClick={(e) => this.props.handleDefaultBookMark(e,this.props.aceVideos)}><i className="fa fa-bookmark" aria-hidden="true"></i></span>
              <span className="icon-btn" onClick={(e) => this.props.handleDefaultFavorite(e,this.props.aceVideos)}><i className="fa fa-heart" aria-hidden="true"></i></span>
            </div>
        </div>
      )
    });
      ...
      <div style={globalStyles.marginTop} id="Aces">
        <div>
          <h2 style={globalStyles.ChannelTitle}>
            Disc Golf Aces & Nice Shots
            <a href="https://www.youtube.com/playlist?list=PLwpmUfonsaevklzeLsLhPaycdqwEtLzkj" target="_blank" rel="noopener noreferrer" style={globalStyles.ChannelLink}>
              <div style={globalStyles.SubscribeBtn}>Follow</div>
            </a>
          </h2>
        </div>
        <div className="video-grid">
        {this.props.aceVideos.length && <OwlCarousel ref="aces" options={globalStyles.owlOptions}>
          {channelItems}
        </OwlCarousel>}
        </div>
      </div>
